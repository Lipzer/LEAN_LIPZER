...
example (lbf : FnHasLb f) (lbg : FnHasLb g) : FnHasLb fun x ↦ f x + g x := by
  rcases lbf with ⟨a, llbfa⟩
  rcases lbg with ⟨b, llbgb⟩
  use a+b
  -- On utilise un exemple déjà vu --
  intro x
  dsimp
  apply add_le_add
  apply llbfa
...
example {c : ℝ} (ubf : FnHasUb f) (h : c ≥ 0) : FnHasUb fun x ↦ c * f x := by
  sorry  --- C'est un exemple déja vu comme précédemment --
...
example (divab : a ∣ b) (divac : a ∣ c) : a ∣ b + c := by
  ...
  use d + e
  simp only[mul_assoc,ceq,beq,mul_add]
